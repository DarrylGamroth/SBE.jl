...
This is the canonical code generation path: the IR is converted to a Schema
structure which is then used with the existing code generator. This approach:

1. Ensures IR is the canonical intermediate representation
2. Validates that IR roundtrips correctly (IR → Schema → Julia)  
3. Reuses the robust existing code generation infrastructure

Future enhancement: Direct IR token → Julia AST generation without Schema bridge.
"""
function generate_from_ir(ir::IR.IntermediateRepresentation)
    # Convert IR to Schema (validates IR structure)
