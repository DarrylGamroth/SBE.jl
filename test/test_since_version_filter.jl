...
@testset "Since Version Filter" begin
    ...
    enc = SinceVersionFilter.MessageWithSince.Encoder(typeof(buffer))
    SinceVersionFilter.MessageWithSince.wrap_and_apply_header!(enc, buffer, 0; header=header)
    SinceVersionFilter.MessageWithSince.fieldSince0!(enc, Int32(1))

    SinceVersionFilter.MessageHeader.version!(header, UInt16(3))
    dec_v3 = SinceVersionFilter.MessageWithSince.Decoder(typeof(buffer))
    SinceVersionFilter.MessageWithSince.wrap!(dec_v3, buffer, 0)
    ...
    dec_v4 = SinceVersionFilter.MessageWithSince.Decoder(typeof(buffer))
    SinceVersionFilter.MessageWithSince.wrap!(dec_v4, buffer, 0)
