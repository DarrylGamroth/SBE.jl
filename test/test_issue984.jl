...
@testset "Issue984 Group Field SinceVersion" begin
    ...
    enc = Issue984.SimpleMessage.Encoder(typeof(buffer))
    Issue984.SimpleMessage.wrap_and_apply_header!(enc, buffer, 0; header=header)
    ...
    dec = Issue984.SimpleMessage.Decoder(typeof(buffer))
    Issue984.SimpleMessage.wrap!(dec, buffer, 0)
    ...
    dec1 = Issue984.SimpleMessage.Decoder(typeof(buffer))
    dec1.position_ptr = SBE.PositionPointer()
    Issue984.SimpleMessage.wrap!(dec1, buffer, 0, UInt16(0), UInt16(1))
